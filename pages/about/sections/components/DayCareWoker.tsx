...
function DayCareWorker() {
  const { isOpen, onClose, onOpen } = useDisclosure();
  const [selectedOfficial, setSelectedOfficial] = useState<OfficialModel>();

  const getDayCareWorkersOfficials = async (): Promise<OfficialModel[]> => {
    const collectionRef = collection(db, "other_officials");
    const q = query(collectionRef, where("position", "==", "Day Care Worker"));
  ...
  const {
    data: daycareWorkerData,
    isFetched: daycareWorkerFetched,
    refetch: daycareWorkerRefetch,
  } = useQuery({
    queryKey: ["day-care-worker-officials"],
    queryFn: getDayCareWorkersOfficials,
  });
  ...
  return (
    <>
      ...
      <Wrap
        ...
        p="2rem"
      >
        {daycareWorkerData?.map((official) => {
          return (
            <VStack
              key={official.id}
              onClick={() => {
