...
function BarangayUtility() {
  const getUtilityOfficials = async (): Promise<OfficialModel[]> => {
    const collectionRef = collection(db, "other_officials");
    const q = query(collectionRef, where("position", "==", "Barangay Utility"));
  ...
  const {
    ...
    isFetched: utilityFetched,
    refetch: utilityRefetch,
  } = useQuery({
    queryKey: ["utility-officials"],
    queryFn: getUtilityOfficials,
  });
  ...
  return (
    <>
      ...
      <Wrap
        ...
        p="2rem"
      >
        {utilityWorkerData?.map((official) => {
          return (
            <VStack
              key={official.id}
              onClick={() => {
...
export default BarangayUtility;
