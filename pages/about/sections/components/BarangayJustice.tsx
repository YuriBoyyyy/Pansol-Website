...
function BarangayJustice() {
  const getBarangayJusticeOfficials = async (): Promise<OfficialModel[]> => {
    const collectionRef = collection(db, "other_officials");
    const q = query(collectionRef, where("position", "==", "Barangay Justice"));
  ...
  const {
    data: barangayJusticeData,
    isFetched: barangayJusticeFetched,
    refetch: barangayJusticeRefetch,
  } = useQuery({
    queryKey: ["barangay-justice-officials"],
    queryFn: getBarangayJusticeOfficials,
  });
  ...
  return (
    <>
      ...
      <Wrap
        ...
        p="2rem"
      >
        {barangayJusticeData?.map((official) => {
          return (
            <VStack
              key={official.id}
              onClick={() => {
...
export default BarangayJustice;
