...
interface ModalProps {
  isOpen: boolean;
  onClose: () => void;
  residentDataToUpdate: ResidentsModel;
}

function UpdateRecord(props: ModalProps) {
  const { isOpen, onClose, residentDataToUpdate } = props;
  const [firstName, setFirstName] = useState<string>(
    residentDataToUpdate.residentData.name.first_name
  );
  const [lastName, setLastName] = useState<string>(
    residentDataToUpdate.residentData.name.last_name
  );
  const [middleName, setMiddleName] = useState<string>(
    residentDataToUpdate.residentData.name.middle_name
  );
  const [occupation, setOccupation] = useState<string>(
    residentDataToUpdate.residentData.occupation
  );
  const [age, setAge] = useState<number | undefined>(
    residentDataToUpdate.residentData.age
  );
  const [gender, setGender] = useState<string>(
    residentDataToUpdate.residentData.gender || ""
  );
  const [status, setStatus] = useState<string>("");
  const [vaccinationStatus, setVaccinationStatus] = useState<string>(
    residentDataToUpdate.residentData.vaccination_status || ""
  );
  const [purok, setPurok] = useState<string>(
    residentDataToUpdate.residentData.purok || ""
  );
  const [religion, setReligion] = useState<string>("");
  const [registeredVoter, setRegisteredVoter] = useState<boolean>(
    residentDataToUpdate.residentData.registered_voter || false
  );
  const [isPWD, setIsPWD] = useState<boolean>(
    residentDataToUpdate.residentData.pwd || false
  );
  const [isIlliterate, setIsIlliterate] = useState<boolean>(
    residentDataToUpdate.residentData.illiterate || false
  );
  const [precinct, setPrecinct] = useState<string>(
    residentDataToUpdate.residentData.precinct || ""
  );

  const [birthday, setBirthday] = useState<string>(
    residentDataToUpdate.residentData.birthday || ""
  );
  const [birthPlace, setBirthPlace] = useState<string>(
    residentDataToUpdate.residentData.birthPlace || ""
  );
  const [educationalAttainment, setEducationalAttainment] =
    useState<string>("");
  const [is4ps, setIs4ps] = useState<boolean>(
    residentDataToUpdate.residentData.is4ps || false
  );
  ...
  const handleSubmit = async () => {
    ...
    const collectionRef = doc(db, "residents", residentDataToUpdate.id);
    await updateDoc(collectionRef, data)
      .then((res) => {
        console.log(res);
        toast({
          title: "Record Updated.",
          description: "We've updated the record for you.",
          status: "success",
          duration: 3000,
          isClosable: true,
          position: "top",
  ...
  return (
    ...
    >
      <ModalOverlay />
      <ModalContent>
        <ModalHeader>Update Resident Record</ModalHeader>
        <ModalCloseButton />
        <ModalBody>
